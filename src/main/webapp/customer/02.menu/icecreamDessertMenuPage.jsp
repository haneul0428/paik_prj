...
<%
	...
	List<ItemVO> breadIcecreamItems = new ArrayList<ItemVO>();	//빵 + 아이스크림
	breadIcecreamItems.addAll(breadItems);
	breadIcecreamItems.addAll(icecreamItems);
	
	Random ranItem = new Random();
	int selectItemRan = Math.min(5, breadIcecreamItems.size());	//최대 5개의
	List<Integer> selectInd = new ArrayList<Integer>();
	
	while(selectInd.size()<selectItemRan){
		int ranInd = ranItem.nextInt(breadIcecreamItems.size());
...
<div class="sub_visual bg-menu4">
	<div class="txt">
		...
		<div class="tabMenu">
			<ul class="pageTab">
				<li><a href="http://localhost/paik_prj/customer/02.menu/newMenuPage.jsp">신메뉴</a></li>
				...
				<li class="on"><a href="http://localhost/paik_prj/customer/02.menu/icecreamDessertMenuPage.jsp">아이스크림/디저트</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="sub_section menu_wrap">
	<div class="mainContainer">
		...
		<div class="swiper-container">
			<div class="swiper-wrapper">
				...
				for(int i=0; i<selectInd.size(); i++){
					...
					ItemVO item = breadIcecreamItems.get(ind);
				%>
		...
		<div class="menuList">
			<ul class="menuItems">
				<% for(ItemVO item : breadIcecreamItems){ %>
				<li data-item-num="<%= item.getItemNum()%>">
					<img src="<%= item.getImage() %>" class="menuItem" />
					<p><strong><%= item.getiNameKor() %></strong></p>
				</li>
