		...
		<div id="findPassContainer">
			<c:choose>
				...
				<p style="color: red; margin-bottom: 10px;">${errorMsg}</p>
					<p class="txt">입력하신 아이디나 이름, 휴대폰 번호를 확인하고 다시 시도해주세요.</p>
				</c:when>
				<c:when test="${not empty tempPassword}">
					<p class="txt">임시 비밀번호가 발급되었습니다.</p>
					<p class="txt" style="margin-bottom: 10px;">회원님의 임시 비밀번호는 <span class="resultTxt">${ tempPassword }</span>입니다.</p>
				</c:when>
			</c:choose>
