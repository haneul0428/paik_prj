...
function validatePhone() {
	...
	if(phone2 == ""){
		alert("휴대폰 번호 중간 자리를 입력해주세요");
		return;
	}//end if
	
	if (phone2.length != 4 || !/^\d{4}$/.test(phone2)) {
		alert("휴대폰 번호 중간자리는 숫자 4자입니다.");
		return;
	}//if
	...
	<div class="container" style="max-width: 600px; margin: 0 auto;">
		...
		<form action="mypage_update_process.jsp" id="updateForm" method="post" style="margin-top: 50px;">
			...
			<div class="status">
				...
				<input type="text" id="name" class="inputBox" value="${ cVO.name }"/>
			</div>
			<div class="status">
				<label for="name">비밀번호</label>
				<input type="button" class="btn btn-warning" id="changePass" value="비밀번호 변경" style="margin-top: 15px; margin-bottom: 15px;">
			</div>
			<div class="status">
				<label for="email">이메일</label>
				<input type="email" id="email" name="email" class="inputBox" value="${ cVO.email }" />
			</div>
			<div class="status">
